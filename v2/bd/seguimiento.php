...
*/
    ...
    /*$query = "SELECT  cra.id_reporte , cra.folio,cra.subcategoria, cra.estatus, sr.seguimiento, CONCAT('https://reporta.zapopan.gob.mx/uploads/evidencias/', sr.foto)  as foto, sr.created_at  as fecha  
              ...
              $query = "SELECT  cra.id_reporte , cra.folio,cra.subcategoria, cra.estatus,
              CASE
                      WHEN sr.seguimiento LIKE '%¡Hola%' THEN
                          CONCAT(
                              SUBSTRING(sr.seguimiento, 1, INSTR(sr.seguimiento, '¡Hola') + LENGTH('¡Hola') - 1), '👋<b', 
                              SUBSTRING(sr.seguimiento, INSTR(sr.seguimiento, '👋') + LENGTH('👋'))
                          )
                      ELSE
                          sr.seguimiento
                  END AS nuevo_seguimiento,
                  
              CONCAT('https://reporta.zapopan.gob.mx/uploads/evidencias/', sr.foto)  as foto, sr.created_at  as fecha  
              FROM SeguimientoReporte sr   join CaratulaReporte cra 
              WHERE  sr.id_reporte  = cra.id_reporte AND  sr.id_reporte = $id_reporte
              ORDER BY sr.created_at ASC";    
